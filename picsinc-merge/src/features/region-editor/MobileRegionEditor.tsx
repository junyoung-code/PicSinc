...
export default function MobileRegionEditor({ imageUrl, width, height, detection, initialMaskUrl, initialSelectedIds = [], busy = false, onSave, saveLabel, participantNickname, saveActionLabel = "다음", onDirtyChange, onRetryDetection, detectionError, onDetailOpen }: MobileRegionEditorProps) {
  ...
  function changed() { if (expandedRef.current) detailDirty.current = true; dirtyRef.current = true; setDirty(true); dirtyCallback.current?.(true); }
  ...
  useEffect(() => {
    ...
    const keydown = (event: KeyboardEvent) => {
      if (event.key === "Escape") { event.preventDefault(); requestDetailExit(); return; }
      if (event.key !== "Tab") return;
      const controls = [...(rootRef.current?.querySelectorAll<HTMLElement>("button:not(:disabled), input:not(:disabled), [tabindex='0']") ?? [])];
  ...
  function openEditor() {
    ...
    detailDirty.current = false; detailComplete.current = false; exitPending.current = false;
  ...
  function leaveEditorFromHistory() {
    ...
    if (!detailComplete.current && detailDirty.current && !window.confirm("상세 편집 중인 변경사항은 저장되지 않습니다. 나갈까요?")) {
  ...
  function draw() {
    ...
    if (tool === "select") for (const [i, region] of regionRef.current.entries()) {
      context.strokeStyle = colors[i % colors.length]; context.lineWidth = (selectedIds.includes(region.id) ? 2.5 : 1.5) / view.scale;
      for (const ring of region.contours) { path(ring); context.stroke(); }
    }
    if (tool === "points") for (const ring of contoursRef.current) {
      ...
      context.save(); context.strokeStyle = "#6558e8"; context.lineWidth = 2 / view.scale;
      context.setLineDash([6 / view.scale, 5 / view.scale]); path(display); context.stroke(); context.restore();
      const visible = spacedHandleIndices(ring, view.scale);
      if (pending?.ring === ring && !visible.includes(pending.index)) visible.push(pending.index);
      for (const index of visible) {
        const p = display[index]; context.beginPath(); context.arc(p.x, p.y, 3.5 / view.scale, 0, Math.PI * 2);
        context.fillStyle = "white"; context.fill(); context.strokeStyle = "#6558e8"; context.lineWidth = 1.5 / view.scale; context.stroke();
      }
    }
  }
  redraw.current = draw;
  ...
  useEffect(() => { draw(); }, [revision, tool, selectedIds, expanded, loading]);
  ...
  function toggle(region: Region) {
    if (locked) return;
    const remove = selectedIds.includes(region.id); const ids = remove ? selectedIds.filter(id => id !== region.id) : [...selectedIds, region.id];
    ...
    setSelectedIds(ids); changed(); refreshMask();
  }
  function down(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (tool === "points") {
      ...
      for (const ring of contoursRef.current) for (const index of spacedHandleIndices(ring, viewRef.current.scale)) {
  ...
  function move(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (pointers.current.size >= 2 && pinch.current) {
      ...
      viewRef.current = view; setZoom(scale / fitScale); draw(); return;
    }
  ...
  function end(event: React.PointerEvent<HTMLCanvasElement>, cancelled = false) {
    ...
    if (current.pending) { maskRef.current = finishContourGesture(current.before, width, height, current.pending, !current.moved); if (current.moved) changed(); refreshMask(false); }
    else if (tool === "brush" || tool === "erase") { changed(); refreshMask(); }
    else if (tool === "select" && !current.moved) {
      const hit = viewportToImage(local(event), viewRef.current);
      if (hit.x >= 0 && hit.y >= 0 && hit.x < width && hit.y < height) { const region = regionRef.current.find(region => region.mask[Math.floor(hit.y) * width + Math.floor(hit.x)]); if (region) toggle(region); }
    }
  }
  function changeZoom(factor: number) {
    const rect = canvasRef.current?.getBoundingClientRect(); if (!rect) return;
    const fitScale = Math.min(rect.width / width, rect.height / height), scale = Math.max(fitScale, Math.min(fitScale * 8, viewRef.current.scale * factor));
    viewRef.current = zoomAround(viewRef.current, { x: rect.width / 2, y: rect.height / 2 }, scale); setZoom(scale / fitScale); draw();
  }
  async function save() {
    if (locked) return;
  ...
  const controls = <div className={styles.tools} aria-label="영역 편집 도구">
    {([['select', '사람 선택'], ['points', '윤곽선 점'], ['pan', '이동'], ['brush', '브러시'], ['erase', '지우기']] as [Tool, string][]).map(([value, label]) => <button key={value} type="button" aria-pressed={tool === value} disabled={locked} onClick={() => setTool(value)}>{label}</button>)}
  </div>;
  return <section ref={rootRef} className={`${styles.editor} ${expanded ? styles.expanded : ""}`} role={expanded ? "dialog" : undefined} aria-modal={expanded || undefined} aria-label="내 영역 선택">
    ...
    <div className={styles.identity}>
      {participantNickname && <strong>{participantNickname}으로 참여 중</strong>}
    </div>
    <p className={styles.hint}>{tool === "points" ? "윤곽선의 흰 점을 끌어 조정하세요. 두 손가락으로 확대·이동할 수 있어요." : "사진 속 사람을 누르면 선택, 다시 누르면 해제됩니다."} {expanded ? "‘완료’를 누르면 이 화면에 반영돼요." : `‘${saveActionLabel}’을 눌러야 저장됩니다.`}</p>
    <div className={styles.canvasFrame}><canvas ref={canvasRef} className={styles.canvas} style={expanded ? undefined : { aspectRatio: `${width} / ${height}` }} aria-label="사진 속 사람 영역을 눌러 선택하거나 해제할 수 있습니다." onPointerDown={down} onPointerMove={move} onPointerUp={event => end(event)} onPointerCancel={event => end(event, true)} onLostPointerCapture={event => { if (pointers.current.has(event.pointerId)) end(event, true); }} /></div>
    {expanded && <div className={styles.zoom}><button type="button" onClick={() => changeZoom(1 / 1.5)} aria-label="축소">−</button><span>{Math.round(zoom * 100)}%</span><button type="button" onClick={() => changeZoom(1.5)} aria-label="확대">+</button><button type="button" onClick={fit}>전체 보기</button></div>}
    <div className={styles.scrollControls}>
      {expanded && controls}
      {!expanded && <button type="button" className={styles.refine} disabled={locked} onClick={openEditor}>영역 직접 다듬기</button>}
      ...
      {(loading || busy) && <p role="status">사진과 영역을 준비하고 있어요.</p>}
    ...
    {!expanded && <button type="button" className={styles.save} disabled={locked || !imageRef.current} onClick={() => void save()}>{saving ? "저장 중…" : saveLabel}</button>}
  </section>;
}
