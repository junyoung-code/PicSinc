...
try {
 ...
 if(process.env.PICSINC_TEST_EXTERNAL_WORKER==='1') {
  const deadline=Date.now()+180_000;
  while(Date.now()<deadline) {
   const state:{data:{status:string}|null;error:unknown}=await db.from('processing_jobs').select('status').eq('id',owned.data!.id).single();assert.equal(state.error,null);
 ...
 const finished=await db.from('processing_jobs').select('status,error_code,result').eq('id',owned.data!.id).single();assert.equal(finished.data?.status,'ready',finished.data?.error_code);
 console.log(JSON.stringify({actualYolo:true,externalWorker:process.env.PICSINC_TEST_EXTERNAL_WORKER==='1',status:finished.data!.status,width:finished.data!.result.width,height:finished.data!.result.height,regions:finished.data!.result.regions.length}));
}finally{
